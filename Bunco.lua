    ...
    add = function(self, card)
        for _, deck_card in pairs(G.playing_cards) do
            if deck_card.config.center == G.P_CENTERS.m_gold then
                deck_card.ability.h_mult = safe_arithmetic(deck_card.ability.h_mult, 0) + card.ability.extra.mult
            end
        end
        G.P_CENTERS.m_gold.config.h_mult = safe_arithmetic(G.P_CENTERS.m_gold.config.h_mult, 0) + card.ability.extra.mult
    end,
    remove = function(self, card)
        for _, deck_card in pairs(G.playing_cards) do
            if deck_card.config.center == G.P_CENTERS.m_gold then
